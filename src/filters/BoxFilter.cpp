  ...
  class BoxFilter : public filters::FilterBase<sensor_msgs::msg::PointCloud2>
  {
    ...
    public:
      ...
      bool configure()
      {
        ...
        this->getParam("max_x", m_maxX, 100);
        this->getParam("max_y", m_maxY, 100);
        this->getParam("max_z", m_maxZ, 100);
        this->getParam("min_x", m_minX, -100);
        this->getParam("min_y", m_minY, -100);
        this->getParam("min_z", m_minZ, -100);
        ...
        sensor_msgs::msg::PointCloud2& output_msg)
      {
        ...
        if(m_buffer == nullptr)
        {
          ...
          if(m_frameId != "" && m_frameId != input_msg.header.frame_id)
          {
            ...
            try
            {
              ...
              {
                RCLCPP_WARN_STREAM(this->logging_interface_->get_logger(), "  BoxFilter is waiting for transform from " << input_msg.header.frame_id << " to " << m_frameId << " to become available.");
                output_msg = input_msg;
                return false;
              }
            }
            ...
            {
              RCLCPP_WARN_STREAM(this->logging_interface_->get_logger(), "  BoxFilter Thread: " << ex.what());
              output_msg = input_msg;            
              return false;
            }
          }
        ...
        boxFilter.setNegative(m_invert);
        boxFilter.setMin(Eigen::Vector4f(m_minX, m_minY, m_minZ, 1.0));
        boxFilter.setMax(Eigen::Vector4f(m_maxX, m_maxY, m_maxZ, 1.0));
